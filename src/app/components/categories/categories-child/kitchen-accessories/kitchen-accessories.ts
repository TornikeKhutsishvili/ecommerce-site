...
@Component({
  ...
  styleUrls: ['./kitchen-accessories.scss']
})
export class KitchenAccessories implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('kitchen-accessories')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'kitchen-accessories');
