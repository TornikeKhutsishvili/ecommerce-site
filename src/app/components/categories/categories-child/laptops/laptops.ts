...
@Component({
  ...
  styleUrls: ['./laptops.scss']
})
export class Laptops implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('laptops')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'laptops');
