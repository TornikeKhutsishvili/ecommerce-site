...
@Component({
  ...
  styleUrls: ['./womens-bags.scss']
})
export class WomensBags implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('womens-bags')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'womens-bags');
