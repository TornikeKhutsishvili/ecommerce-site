...
@Component({
  ...
  styleUrls: ['./sunglasses.scss']
})
export class Sunglasses implements OnInit {
  ...
  filteredProducts = computed(() => {
    ...
    products = products.filter(p => p.category === 'sunglasses');

    return products;
  });

  ngOnInit() {
    this.productService.getProductsByCategory('sunglasses')
