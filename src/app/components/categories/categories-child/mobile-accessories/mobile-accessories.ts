...
@Component({
  ...
  styleUrls: ['./mobile-accessories.scss']
})
export class MobileAccessories implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('mobile-accessories')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'mobile-accessories');
