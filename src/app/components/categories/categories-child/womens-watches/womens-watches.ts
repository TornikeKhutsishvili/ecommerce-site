...
@Component({
  ...
  templateUrl: './womens-watches.html',
  styleUrls: ['./womens-watches.scss']
})
export class WomensWatches implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('womens-watches')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'womens-watches');
