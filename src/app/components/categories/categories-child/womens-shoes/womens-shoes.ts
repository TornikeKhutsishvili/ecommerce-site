...
@Component({
  ...
  templateUrl: './womens-shoes.html',
  styleUrls: ['./womens-shoes.scss']
})
export class WomensShoes implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productService.getProductsByCategory('womens-shoes')
    ...
    this.filterService.filteredProducts$
      ...
      .subscribe(filtered => {
        if (filtered.length > 0) {
          ...
          const categoryFiltered = filtered.filter(p => p.category === 'womens-shoes');
